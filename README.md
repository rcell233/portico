...
5. 点击“打开应用”会保存并进入页面；侧栏自动收起，通过返回箭头回到主机和应用列表。关闭标签页或退出 Portico 后，远端托管服务继续运行。
...
- 配置的服务始终通过 SSH 访问；外部 CDN、CSS、脚本、字体和图片通过本机公网连接加载，不按域名硬编码。外部请求不能连接本机或私网地址；页面导航仍限制在配置的服务内，跨域 SSO 和跨端口导航暂不支持。
