...
页面导航限制在配置地址内，服务请求始终走 SSH。外部 HTTP(S) / WebSocket 资源由独立公网连接加载，DNS 解析后检查并固定目标 IP，禁止转入本机或私网；无域名白名单。外部 SSO / 跨端口导航暂不支持。新窗口请求转为同一应用内的合法导航，权限请求默认拒绝，文件下载由原生保存对话框处理。
