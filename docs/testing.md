...
- 空名称应用的首次网页标题持久化、并发标题去重、手动名称保护、旧页面事件与编辑 / 删除的竞争。
...
运行以下命令打开隔离测试工作空间，不读取或修改用户的 SSH 配置。选择 `proxy-fixture` 应直接进入端口表单，读取端口时出现系统 SSH 的指纹和密码提示。测试密码为 `test-secret`。选中测试端口、名称留空后点击“打开应用”，应显示“实验看板 · Portico QA”；返回工作空间后确认左侧树和卡片同步显示该名称。改变网页标题不应覆盖首次保存的名称。点击主机旁“＋”应直接打开固定主机的端口表单。关闭测试窗口会清理临时配置与连接。
